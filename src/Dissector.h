  ...
  private:
    ...
    void DissectField(tvbuff_t *tvb, proto_item *mainItem, proto_tree *mainTree, const ParsedField* parsedField, int offset)
    {
        if (TryDissectBits(tvb, mainItem, parsedField, offset)) {
            return;
        } else if (TryDissectDecimal128(tvb, mainTree, parsedField, offset)) {
            return;
        } else if (TryDissectMessage(tvb, mainTree, parsedField, offset)) {
            return;
        } else if (TryDissectArray(tvb, mainTree, parsedField, offset)) {
            return;
        } else if (TryDissectStringPointerType(tvb, mainTree, parsedField, offset)) {
            return;
        } else if (TryDissectPointer(tvb, mainTree, parsedField, offset)) {
            return;
        } else if (TryDissectUnion(tvb, mainTree, parsedField, offset)) {
            return;
        } else {
    ...
    bool TryDissectBits(tvbuff_t *tvb, proto_item *mainItem, const ParsedField* parsedField, int offset)
    {
        ...
        proto_tree_add_bitmask(mainItem, tvb, fieldOffset, hf, ett,(const int**) bits.data(), ENC_LITTLE_ENDIAN);
    ...
    bool TryDissectDecimal128(tvbuff_t *tvb, proto_tree *mainTree, const ParsedField* parsedField, int offset)
    {
        ...
        uint128.lo = tvb_get_guint64(tvb, fieldOffset, ENC_LITTLE_ENDIAN);
        uint128.hi = tvb_get_guint64(tvb, fieldOffset + sizeof(uint64_t), ENC_LITTLE_ENDIAN);
    ...
    uint64_t GetArrayCount(tvbuff_t *tvb, const ParsedField* parsedField, int offset)
    {
        ...
        {
            case 1: return tvb_get_guint8(tvb, fieldOffset);
            case 2: return tvb_get_guint16(tvb, fieldOffset, ENC_LITTLE_ENDIAN);
            case 4: return tvb_get_guint32(tvb, fieldOffset, ENC_LITTLE_ENDIAN);
            case 8: return tvb_get_guint64(tvb, fieldOffset, ENC_LITTLE_ENDIAN);
            default: return 0;
        }
    }

    bool TryDissectStringPointerType(tvbuff_t *tvb, proto_tree *mainTree, const ParsedField* parsedField, int offset)
    {
        if (parsedField->isStringPointer == false)
            return false;

        const auto hf = _hfHandler.GetIntByIndex(parsedField->hfIndex);

        uint64_t pointerValue = tvb_get_guint64(tvb, offset + parsedField->offset, ENC_LITTLE_ENDIAN);
        tll_scheme_offset_ptr_t* offsetPtr = reinterpret_cast<tll_scheme_offset_ptr_t*>(&pointerValue);

        const auto fieldOffset = offset + parsedField->offset + offsetPtr->offset;
        const auto fieldSize = offsetPtr->size;
    ...
    bool TryDissectPointer(tvbuff_t *tvb, proto_tree *mainTree, const ParsedField* parsedField, int offset)
    {
        ...
        for (uint32_t i = 0; i < offsetPtr->size; i++) {
            auto fieldOffset = offset + parsedField->offset + offsetPtr->offset;
            fieldOffset += i * parsedField->pointerField->size;
            fieldOffset += (offsetPtr->entity == 0xFF) ? 4 : 0; // if entity is more than 255 (0xFF) than extra 4 bytes before used to store size
    ...
    bool TryDissectUnion(tvbuff_t *tvb, proto_tree *mainTree, const ParsedField* parsedField, int offset)
    {
        ...
        uint8_t tagValue = tvb_get_guint8(tvb, fieldOffset);
